...
export default function FooterCategories({ categories }: FooterCategoriesProps) {
  return (
    <div className="sm:px-5 md:px-[25px] xl:pt-0 xl:mt-0 lg:py-28 hidden mx-auto md:block md:py-5 px-8 md:max-w-3xl lg:max-w-5xl align-baseline pb-6 mb-10 pt-14">
      <h4 className="xl:relative font-secondary italic tracking-wider relative left-6 -top-4 z-2 py-1 px-6 border border-solid rounded-full text-lg bg-white leading-relaxed mb-7 inline-block">
        Categories
      </h4>
      ...
      {categories.map((category: BlogCategory, index: number) => ( 
        <Link 
          href="#" 
          className={`rounded-full inline-block py-1 px-4 mt-0 mr-2 mb-3 ml-0 font-primary text-sm tracking-wide align-baseline break-words bg-white border border-solid cursor-pointer border-yellow-green-500 text-zinc-700 ${colors[index].border} ${colors[index].hover}`}
          key={category.id}>

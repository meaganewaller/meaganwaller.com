  ...
  return (
    <>
      <motion.div 
        ...
        className="flex fixed z-10 justify-between items-center py-4 px-6 w-full leading-5 text-yellow-green-500 bg-emerald-900">
        <Link
          href="/"
          aria-current="page"
        ...
        <div className="flex text-yellow-green-500">
          {nav.main.map((item) => {
            return <NavItem href={item.href} text={item.title} target={item.target} key={item.title} />
          })}
        </div>
      </motion.div>
      <div className="pt-6 mb-0 leading-6 bg-emerald-900 text-yellow-green-500">

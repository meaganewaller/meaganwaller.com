...
const discussURL = (title: string, slug: string) => {
  return `https://github.com/${meta.repo.name}/discussions/new?category=post&title=${title}`;
};
...
const BlogSeo = ({ title, description, publishedAt, url, image }) => {
  ...
  const featuredImage = {
    url: image?.url ? "https://proxy.meaganwaller.com/rewrite?url=" + Buffer.from(image.url).toString("base64") : config.baseUrl + "/og.png",
    alt: title,
    width: image?.width,
    height: image?.height,
    type: image?.type,
      ...
      <article className="mx-auto mb-16 flex w-full max-w-4xl flex-col items-start justify-center">
        ...
        {cover && (
          <div className="mb-8 flex h-72 w-full items-center overflow-hidden rounded align-middle">
            <Image
              src={"https://proxy.meaganwaller.com/?rewrite=" + Buffer.from(cover.url).toString("base64")}
              alt=""
              width={cover.width}
              height={cover.height}
              placeholder="blur"

...
import type { QueryDatabaseParameters, QueryDatabaseResponse } from "@notionhq/client"
...
export async function blogIndex(): Promise<Blog[]> {
  ...
  if (!id) {
    throw new Error('NOTION_BLOG_DATABASE_ID not set');
  }

  return (await index(id, { property: 'Date', direction: 'descending' }))
    .map(({ common, row }) => ({
      ...common,
      author: (getPropertyData(row.properties.Author as Property<'person'>)[0] as { name: string }).name,
...
// // export async function getBlogIndex(): Promise<{
// //   [key: string]: { id: string; type: string; children?: { id: string; type: string; slug: string }[] }
// // }> {
// //   const { results: root } = await getDatabase(process.env.NOTION_BLOG_DATABASE_ID);
...
// // }
// // //   }
// // //     children?: { id: string; type: string; slug: string }[];
// // //   };
// // // }> {
// // //   const { results: root } = database;
// // //
// // //   return (
// // //     await Promise.all(
// // //       root.map(
// // //         async ({ properties: { name, page: pageId } }: PageWithChildren) => {
// // //           const pageName = name[name.type]
// // //     .map(({ plain_text }) => plain_text)
// // //     .join('');
// // //   const page = pageId[pageId.type].find(
// // //     ({ type }) => type === 'mention',
// // //   ).mention;
// // //
// // //   const children =
...
// // //         ({ properties, id, object }: PageWithChildren) => {
// // //           return {
// // //             id,
// // //             type: object,
// // //             slug: slugify(
// // //               properties.title[properties.title.type].map(
...
// // //     list.results
// // //       .filter(
// // //         ({ has_children, type }: BlockWithChildren) =>
// // //           !['unsupported', 'child_page'].includes(type) && has_children,
// // //       )
// // //       .map(async ({ id }) => {
// // //         return { id, children: await getBlockChildren(id) };
// // //       }),
...
// // //     ) {
// // //       block[block.type].children = children.find(
// // //         ({ id }) => id === block.id,
// // //       )?.children;
// // //     }
// // //     return block;
...
// // //             .map((block: BlockWithChildren) => {
// // //               return {
// // //                 title: block[block.type].rich_text
// // //                 .map(({ plain_text }) => plain_text)
// // //                 .join(''),
// // //                 type: block.type,
// // //                 children: [],
...
// // //                 const prev = acc[acc.length - 1];
// // //                 const prevprev = prev?.children[prev.children.length - 1];
// // //                 if (prevprev.type?.type == 'heading_2') {
// // //                   prevprev.children.push(curr);
// // //                 } else if (prev?.type === 'heading_1') {
// // //                   prev.children?.push(curr);
// // //                 } else {
...
// // //             properties: { title },
// // //           } = await getPage(contents[contents.type]);
// // //           block.link_to_page['title'] = title[title.type]
// // //             .map(({ plain_text }) => plain_text)
// // //             .join('');
// // //           break;
// // //
...
// // //             title:
// // //             og_data.title ||
// // //               og_data.twitter_card?.title ||
// // //               og_data.open_graph?.title ||
// // //               null,
// // //             description:
// // //             og_data.description || og_data.open_graph?.description || null,
// // //             url: contents.url,
// // //           };
// // //
// // //           if (image && (image.secure_url || image.url)) {
...
// // //               base64,
// // //               img: { height, width },
// // //             } = await getPlaiceholder(image.secure_url || image.url, {
// // //                 size: 64,
// // //               });
// // //
// // //             block[block.type]['meta'].image = {
...
// // //         case 'synced_block':
// // //           if (contents.synced_from != null) {
// // //             const source_block = await getBlockChildren(
// // //               contents.synced_from.block_id,
// // //             );
// // //             block[block.type]['children'] = source_block;
// // //           }
// // //           break;

...
const notion_color = {
  ...
  gray_background:
    '!bg-opacity-10 !bg-gray-500 !border-gray-200 dark:!border-gray-900',
  brown_background:
    '!bg-opacity-10 !bg-stone-500 !border-stone-200 dark:!border-stone-900',
  orange_background:
    '!bg-opacity-10 !bg-orange-500 !border-orange-200 dark:!border-orange-900',
  yellow_background:
    '!bg-opacity-10 !bg-yellow-500 !border-yellow-200 dark:!border-yellow-900',
  green_background:
    '!bg-opacity-10 !bg-emerald-500 !border-emerald-200 dark:!border-emerald-900',
  blue_background:
    '!bg-opacity-10 !bg-sky-500 !border-sky-200 dark:!border-sky-900',
  purple_background:
    '!bg-opacity-10 !bg-indigo-500 !border-indigo-200 dark:!border-indigo-900',
  pink_background:
    '!bg-opacity-10 !bg-pink-500 !border-pink-200 dark:!border-pink-900',
  red_background:
    '!bg-opacity-10 !bg-rose-500 !border-rose-200 dark:!border-rose-900',
};
  ...
  switch (block.type) {
    case 'mention':
      ...
      switch (contents.type) {
        case 'date':
          ...
          return (
            <time dateTime={start.toISOString()}>
              ...
              {date.end
                ? ` - ${hasTime(end) ? withTime(end) : withoutTime(end)}`
                : ''}
            </time>
          );
        case 'user':
          return (
            <Mention
              type="user"
              link={`mailto:${mention.person.email}`}
              text={mention.name}
            />
          );
        case 'page':
        ...
        case 'link_preview':
          const GHreg =
            /https?:\/\/github\.com\/(?<user>[^\/\s]+)\/(?<repo>[^\/\s]+)\/?((issues|pull)\/(?<num>\d+))?/;
        ...
        case 'template_mention':
          return (
            <Mention
              type={mention.type.replace('template_mention_', '')}
              link={null}
              text={mention[mention.type]}
            />
          );
    ...
    case 'text':
      ...
      const highlight = color.includes('background')
        ? ' p-[0.125rem] rounded-sm dark:bg-opacity-40 bg-opacity-50'
        : '';

      let part: JSX.Element = contents.link ? (
        <Link href={contents.link.url}>{contents.content}</Link>
      ) : (
        <>{contents.content}</>
      );
      ...
      if (strikethrough)
        part = <del className="text-gray-400 dark:text-gray-600">{part}</del>;
      if (underline) part = <u>{part}</u>;
      if (color.includes('background'))
        part = <mark className="bg-transparent text-inherit">{part}</mark>;

      return (
        <span
          className={(classes[color] || '') + highlight}
  ...
  switch (block.type) {
    ...
    case 'numbered_list_item':
      ...
      return (
        <li
          className={
            'my-2 rounded-sm px-2 py-1 last:!mb-0 last:!pb-0 ' +
            notion_color[contents.color || 'default']
          }
        >
          <NotionText blocks={contents.rich_text} />
    ...
    case 'to_do':
      ...
      return (
        <label
          htmlFor={block.id}
          className={
            'my-2 block rounded-sm py-1 pl-9 -indent-6 ' +
            notion_color[contents.color || 'default']
          }
        >
          <input
            ...
            disabled
            className="mr-2.5 scale-125 align-middle"
          />
          <span
            className={
              contents.checked
    ...
    case 'callout':
      ...
      return (
        ...
        >
          <div
            className="mt-0.5 h-6 w-6 select-none overflow-hidden rounded"
            aria-hidden="true"
          >
            {contents.icon.type == 'emoji' ? (
              ...
              <Image
                alt=""
                src={
                  'https://proxy.maximousblk.me/rewrite?url=' +
                  Buffer.from(icon[icon.type].url).toString('base64')
                }
                height={24}
                width={24}
              />
        ...
        <figure>
          <div className="mx-auto flex w-fit overflow-hidden rounded-md border border-gray-200 bg-gray-100 dark:border-gray-800 dark:bg-gray-800">
            <Image
              ...
              src={
                'https://proxy.maximousblk.me/rewrite?url=' +
                Buffer.from(contents[contents.type].url).toString('base64')
              }
              alt={contents?.caption
      ...
      return (
        <figure>
          <audio
            ...
            preload="auto"
            className="!h-10 max-h-max !w-full max-w-full"
          />
          {contents.caption.length > 0 && (
            <figcaption>
              <NotionText blocks={contents.caption} />
      ...
      return (
        <div className="grid grid-flow-row md:grid-flow-col md:gap-8">
          {contents.children.map(
            ({ column: { children: columnChildren }, id }) => (
    ...
    case 'quote':
      ...
      return (
        <blockquote
          className={'py-1 ' + notion_color[contents.color || 'default']}
        >
          <NotionText blocks={contents.rich_text} />
        </blockquote>
      );
    case 'table':
      ...
      return (
        <table className="w-full table-auto">
          ...
          <tbody>
            {children
              .slice(contents.has_column_header ? 1 : 0)
              .map(({ table_row }, i) => (
                <tr key={i}>
...
function Mention({
  ...
  return (
    <Link
      href={link}
      className="whitespace-nowrap rounded p-1 hover:bg-gray-100 dark:hover:bg-gray-800"
    >
      <MentionIcon
        size={16}
        className="mr-1 mb-0.5 inline-block text-gray-500 dark:text-gray-500"
      />
      <span>{text}</span>
    </Link>
  );
  ...
  return (
    <figure className="my-6">
      ...
      >
        ...
        <span
          className={
            'w-full break-all line-clamp-1' + (mono ? ' font-mono' : '')
          }
        >
          {text}
        </span>
...
> & {
  ...
  return (
    <details
      className={
        'group my-6 rounded border border-gray-200 bg-gray-50 px-3 py-2 dark:border-gray-800 dark:bg-gray-900 ' +
        className
      }
    >
      <summary className="!m-0 flex cursor-pointer list-none space-x-2 font-medium">
        ...
        <p className="m-0 w-full line-clamp-1 group-open:line-clamp-none">
          {summary}
        </p>
      </summary>
      <hr className="mt-2 mb-4" />
      {details || (
        <p className="text-gray-400 dark:text-gray-600">This block is empty</p>
      )}
    </details>
  );
}

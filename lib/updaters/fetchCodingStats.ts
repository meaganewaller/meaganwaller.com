...
import { ConvertBytes } from "@lib/utils/convertBytes";
import { ConvertNumber } from "@lib/utils/convertNumber";
import { getCommits } from "@lib/utils/getCommits";
import { getTotalContributionsForYears } from "@lib/utils/getContributions";
import { getLinesOfCode } from "@lib/utils/getLinesOfCode";
import { getRepositoriesInfo } from "@lib/utils/getRepositoriesInfo";
import { percentageBar } from "@lib/utils/percentage";

export default async function fetchCodingStats(apiToken: string, username: string) {
  ...
  await fetch("https://wakatime.com/api/v1/users/current/stats/last_7_days", {
    headers: {
      Authorization: `Basic ${process.env.WAKATIME_API_KEY.toString("base64")}`,
    },
  })
    .then((res) => res.json())
    .then((data) => {

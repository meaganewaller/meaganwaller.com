    ...
    <div className="h-screen bg-desktopWallpaper2">
      ...
      <Window
        ...
        <div className="flex p-5 place-content-center flex-col">
          ...
          <p className="font-mono text-md text-secondary-dark px-8 leading-relaxed py-2">
          ...
          <div className="flex place-content-end group wavy">
            <NextLink href="/about" className="text-secondary">Read more <BsArrowRightShort size="1.5em" className="inline-block" /> </NextLink>
          </div>
        </div>
      </Window>
            ...
            {posts.map((post: Blog) => (
              <li key={post.slug} className="px-2 wavy w-full">
                <NextLink href={`/blog/${post.slug}`} className="block">
                  ...
                  <span className="italic font-mono lowercase text-accentFirst-lighter">{formatDate(post.publishedAt)}</span>
                </NextLink>
              </li>
            ))}
          </ul>

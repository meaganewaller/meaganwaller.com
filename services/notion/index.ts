import { BlogPostProperties, NotionBlockResponseList, BlogCategory, BlogCategoryProperties } from '@localTypes/schema';
import { Client as NotionClient, iteratePaginatedAPI } from '@notionhq/client';
import { BlockWithChildren, getChildBlocksWithChildrenRecursively } from '@jitl/notion-api';
import { isFullPage } from '@notionhq/client';
import { plainText } from './client';
...
export const getCategoryList = async (): Promise<BlogCategory[]> => {
  ...
  for (const page of results) {
    ...
    const properties: BlogCategoryProperties = page.properties as unknown as BlogCategoryProperties;
...
export const getBlogList = async () => {
  ...
  for await (const page of iteratePaginatedAPI(notion.databases.query, {
    ...
    if (isFullPage(page)) {
      ...
      const properties: BlogPostProperties = page.properties as unknown as BlogPostProperties;

...
async function getCategoryDatabase() {
  const database = await notion.databases.query({
    database_id: process.env.NOTION_CATEGORY_DATABASE_ID as string,
    sorts: [
      {
        property: "Post Count",
        direction: "descending",
...
export async function getBlogPostInfoList(): Promise<BlogPost[]> {
  ...
  for (const page of pages) {
    ...
    const title = titleProps[0]?.text.content ?? "No Title"
    let coverUrl = null
    if (cover) {
...
async function deepFetchAllChildren (blocks: GetBlockResponse[]): Promise<BlockObjectResponse[]> {
  ...
  const results = await Promise.all(fetchChildrenMa.map(value => value.promise))
